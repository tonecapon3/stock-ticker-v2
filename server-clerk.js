...
async function enrichUserInfo(req, res, next) {
  ...
  try {
    ...
    if (auth.userId && clerkClient) {
      ...
      // Set admin role based on metadata or email domain
...
app.delete('/api/remote/stocks/:symbol', requireAuthWithFallback, enrichUserInfo, (req, res) => {
  try {
    if (req.isUnauthenticated) {
      return res.status(401).json({ error: 'Authentication required to delete stocks' });
    }

    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Admin privileges required to delete stocks' });
    }
...
app.post('/api/remote/restart', requireAuthWithFallback, enrichUserInfo, (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Admin privileges required for server restart' });
    }
...
app.listen(PORT, () => {
  ...
  console.log('  DEL  /api/remote/stocks/:symbol     - Delete stock (admin)');
  ...
  console.log('  POST /api/remote/restart            - Restart server (admin)');
  console.log('');
