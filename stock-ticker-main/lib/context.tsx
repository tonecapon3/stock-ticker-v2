...
export const TickerProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
   ...
   * with validation and rate limiting
   */
  const changeCurrency = useCallback((currency: CurrencyCode): ValidationResult => {
    try {
      ...
      // If the currency is already set to the requested one, no change needed
      if (tickerState.currency === currency) {
        return { isValid: true };
      }
  ...
  }, [tickerState.currency, getRateLimiter, safelyUpdateState, setError]);
