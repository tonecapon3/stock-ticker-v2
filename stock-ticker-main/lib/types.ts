...
export const SECURITY_CONSTRAINTS = {
  ...
  ALLOWED_CURRENCIES: ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'INR', 'CHF'],
};

/**
 * Supported currency codes
 */
export type CurrencyCode = 'USD' | 'EUR' | 'GBP' | 'JPY' | 'CAD' | 'INR' | 'CHF';
...
export interface TickerContextType {
  ...
  changeCurrency: (currency: CurrencyCode) => ValidationResult;
  ...
  validateInput: {
    ...
    currency: (currency: string) => ValidationResult;
  };
...
export function validateCurrency(currency: string): ValidationResult {
  ...
  if (!SECURITY_CONSTRAINTS.ALLOWED_CURRENCIES.includes(currency)) {
    return { 
      isValid: false, 
      errorMessage: `Currency must be one of: ${SECURITY_CONSTRAINTS.ALLOWED_CURRENCIES.join(', ')}` 
    };
  }
